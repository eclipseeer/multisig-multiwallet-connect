  ...
  persist(
    {
      ...
      selectAccount: action((state, payload) => {
        state.selectedAccount = state.accounts.find((account) => account.accountId === payload);
      }),
      addAccount: action((state, { accounts }) => {
        const anotherKeysAccounts = state.accounts.filter((a) => a.publicKey !== accounts[0].publicKey);
